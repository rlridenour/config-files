...
(article "#+TITLE: " p n
 ...
 "#+LaTeX_HEADER: % \\usepackage[authordate,url=false,isbn=false,backend=biber]{biblatex-chicago} %Change authordate to notes if desired." n
...
(article "% arara: latexmk: { engine: pdflatex }" n
n "\\documentclass[11pt]{article}" n
n "\\usepackage{graphicx,epstopdf,amsmath,amssymb,url,tabularray}" n
...
"% \\usepackage[authordate,url=false,isbn=false,backend=biber]{biblatex-chicago}" n
"% Change authordate to notes if desired." n
...
n n
n "\\end{truefalse}" n
n "\\begin{multiplechoice}[title={Multiple Choice (2 points each)},resetcounter=yes]" n
"	Circle the correct answer." n
n n
n "\\end{multiplechoice}" n
n n
